...
const nav = [
  ...
  { label: "Learning Progress", href: "/teacher/learning-progress" },
...
const tasks = [
  { title: "Review JSS 2A learning evidence", meta: "Fractions remains the lowest combined topic", tone: "urgent", href: "/teacher/learning-progress" },
  ...
  { title: "Review CBT practice", meta: "38 attempts · topic-level evidence available", tone: "normal", href: "/teacher/cbt" },
];
  ...
  return <main className="teacher-shell">
    ...
    <section className="teacher-main">
      ...
      <div className="teacher-content">
        <section className="teacher-welcome"><div><span className="page-kicker">MONDAY · 14 SEPTEMBER</span><h2>Good morning, Mrs. Amina.</h2><p>You have 4 lessons today, a weekly learning update to publish, and topic-level learning evidence ready for review.</p></div><div className="teacher-quick-actions"><Link href="/teacher/learning-progress">Open learning progress</Link><Link href="/teacher/cbt" className="secondary">Open CBT practice</Link></div></section>

        <section className="teacher-ai-brief"><div className="ai-orb">AI</div><div><div className="ai-heading"><strong>Teacher AI Daily Brief</strong><span>Updated 7:45 AM</span></div><p>JSS 2B is slightly behind syllabus pace and three students show a combined attendance and assessment risk. JSS 2A evidence from classwork, assignments, assessments and CBT shows fractions as the most consistent practice area.</p><div className="ai-buttons"><Link href="/teacher/learning-progress">Review learning evidence</Link><Link href="/teacher/ai">Ask Teacher AI</Link></div></div></section>

        <section className="teacher-kpis"><Kpi label="Today's lessons" value="4" hint="1 completed · 3 upcoming" accent="blue" /><Kpi label="My students" value="150" hint="Across 4 assigned classes" accent="green" /><Kpi label="Topics to review" value="7" hint="Across current classes" accent="amber" /><Kpi label="Syllabus progress" value="71%" hint="+4% from last week" accent="purple" /></section>
        ...
        <section className="teacher-grid equal"><Panel title="My classes" subtitle="Class size, next lesson and curriculum progress"><div className="class-cards">{classes.map((item) => <article key={item.name} className="class-card"><div className="class-card-head"><div><span>{item.subject}</span><strong>{item.name}</strong></div><b>{item.progress}%</b></div><p>{item.students} students · Room {item.room}</p><div className="progress"><i style={{ width: `${item.progress}%` }} /></div><footer><span>Next: {item.next}</span><Link href="/teacher/classes">Open class</Link></footer></article>)}</div></Panel><Panel title="Students needing attention" subtitle="Generated from attendance and academic trends"><div className="student-attention-list">{filteredStudents.map((student) => <div className="student-attention" key={student.name}><div className="student-avatar">{student.name.split(" ").map((x) => x[0]).join("").slice(0,2)}</div><div><strong>{student.name}</strong><span>{student.className} · Avg {student.avg}% · Attendance {student.attendance}%</span></div><span className={`risk-chip ${student.flag.toLowerCase().replace(" ", "-")}`}>{student.flag}</span></div>)}</div><Link className="panel-link" href="/teacher/learning-progress">Open learning evidence →</Link></Panel></section>

        <section className="teacher-grid equal"><Panel title="Planning, weekly learning & evidence" subtitle="One connected teaching workflow"><div className="plan-summary"><div><span>Lesson plans submitted</span><strong>11 / 12</strong></div><div><span>Weekly update</span><strong>Draft ready</strong></div><div><span>CBT sets published</span><strong>8</strong></div><div><span>Evidence sources</span><strong>4</strong></div></div><div className="teacher-note"><strong>Connected workflow</strong><p>Plan the lesson, record what was covered, collect classwork and assignment evidence, add assessment/CBT results, then review the topic trend before deciding the next teaching action.</p><Link href="/teacher/learning-progress">Open Learning Progress</Link></div></Panel><Panel title="My performance" subtitle="Private professional dashboard"><div className="performance-score"><div className="score-ring"><strong>88</strong><span>/100</span></div><div><strong>Very good</strong><p>Your strongest areas are attendance completion and lesson-plan quality. Syllabus pace needs attention in JSS 2B.</p></div></div><Metric label="Attendance completion" value={98} /><Metric label="Lesson-plan compliance" value={92} /><Metric label="Assessment completion" value={84} /><Metric label="Syllabus progress" value={71} /><Link className="panel-link" href="/teacher/performance">Open my performance →</Link></Panel></section>
      </div>
    </section>
  </main>;
}
