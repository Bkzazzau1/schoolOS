...
export default function BoardingPage(){
  ...
  return <main className="school-life-page">
    ...
    <section className="school-life-scope"><div><strong>Boarding & Hostel</strong><span>Optional school module</span></div><p>Schools without boarding can disable this module entirely. Where enabled, it covers dorm capacity, approved leave/return, duty coverage, welfare coordination and facilities issues.</p></section>
    <section className="school-life-stat-grid"><article className="school-life-stat"><span>Module status</span><strong>{enabled?"Enabled":"Off"}</strong><small>School-configurable</small></article><article className="school-life-stat"><span>Dorm occupancy</span><strong>{occupied}/{capacity}</strong><small>Current mock residents</small></article><article className="school-life-stat"><span>On campus</span><strong>120</strong><small>Current prototype count</small></article><article className="school-life-stat"><span>Approved leave</span><strong>5</strong><small>Expected return tracked separately</small></article><article className="school-life-stat"><span>Maintenance items</span><strong>2</strong><small>Facilities follow-up</small></article></section>
    <section className="school-life-grid"><article className="school-life-card"><div className="school-life-section-head"><div><h2>Dormitory overview</h2><p>Operational accountability without continuous or invasive monitoring of students.</p></div><button onClick={()=>setEnabled(v=>!v)}>{enabled?"Disable prototype module":"Enable prototype module"}</button></div>{enabled?<><div className="school-life-filters"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search dorm or house parent..."/></div><div className="activity-list">{visible.map(d=>{const done=reviewed.includes(d.name);return <div className="activity-row" key={d.name}><div className="activity-icon">⌂</div><div><div className="activity-meta"><span>{d.status}</span><span>{d.occupied}/{d.capacity} occupied</span><span>{d.approvedLeave} on leave</span></div><h3>{d.name}</h3><p>House parent: {d.houseParent}</p><p>{d.note}</p><small>{d.onCampus} on campus · {d.maintenance} maintenance items</small><div className="notice-actions" style={{marginTop:8}}><button onClick={()=>setReviewed(c=>c.includes(d.name)?c.filter(x=>x!==d.name):[...c,d.name])}>{done?"Reopen review":"Mark handover reviewed"}</button></div></div><span className="activity-status">{done?"Reviewed":d.status}</span></div>})}</div></>:<div className="school-life-note"><strong>Boarding disabled</strong>This school can hide the module when it is a day school.</div>}</article><aside className="school-life-sidebar"><article className="school-life-card"><span className="school-life-kicker">BOARDING BOUNDARY</span><div className="school-life-policy-list"><div><strong>Welfare, not surveillance</strong><small>Use roll checks, duty handover and approved leave—not intrusive monitoring.</small></div><div><strong>Restricted records</strong><small>Private welfare or health details belong in authorized workflows.</small></div><div><strong>Optional by tenant</strong><small>Day schools should not carry unused hostel navigation.</small></div></div></article></aside></section>
  </main>;
}
